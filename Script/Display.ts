// 
// 1. Updated Interface to match your Course structure
interface Product {
    CourseID: number;
    CourseName: string;
    Image: string;
...
const prodApi: string = "http://localhost:49251/api/Courses";
const enrollApi: string = "http://localhost:49251/api/Enrollments"; // Adjust ports
const invoiceApi: string = "http://localhost:49251/api/Invoice";
...
function displayWatch(containerId: string, catId: number): void {
    $.ajax({
        url: prodApi,
        type: "GET",
        success: (products: Product[]) => {
            const filteredProd = products.filter((item: Product) => item.CategoryID == catId);
 ...
 */
function handleEnrollment(courseID: number, price: number): void {
 ...
 */
function processSuccessfulPayment(courseID: number, amount: number): void {
...
// Global Exports
(window as any).displayWatch = displayWatch;
(window as any).handleEnrollment = handleEnrollment;
