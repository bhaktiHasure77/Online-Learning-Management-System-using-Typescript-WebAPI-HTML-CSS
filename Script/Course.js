"use strict";
// 2. API Endpoints
const courseApi = "http://localhost:49251/api/Courses";
const catApi = "http://localhost:49251/api/Category";
class CourseManager {
    categoryList = [];
    UserID;
    constructor() {
    ...
    getData() {
        ...
        const category = parseInt($("#ddlCategory").val());
        const price = parseFloat($("#txtPrice").val());
        const img = $("#txtImage").attr("data-path");
        const desc = $("#courseDescription").val();
        if (!name) {
        ...
        return {
            CourseID: parseInt($("#ddlcourseID").val()) || 0,
            CourseName: name,
            CategoryID: category,
            Image: img,
            Price: price,
...
window.addCourse = () => { manager.clearForm(); manager.enableForm(); };
window.Edit = () => manager.enableForm();
window.Save = () => manager.save();
window.updateCourse = () => manager.update();
window.deleteCourse = () => manager.delete();
